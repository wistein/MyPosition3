...
class EarthGravitationalModel : VerticalTransform() {
    ...
    // WGS 84 Earths Gravitational Constant w/ atmosphere.
    ...
    */
    ...
    fun load(context: Context) {
        val stream = context.resources.openRawResource(R.raw.egm180)
        val `in` = LineNumberReader(InputStreamReader(stream, StandardCharsets.ISO_8859_1))
        var line: String?
    ...
    private fun initialize() {
        ...
        for (i in 0..nmax) {
            for (j in i + 1..nmax) {
                ...
                bClenshaw[ll] = sqrt(n * (j + i - 1) * (j - i - 1) / (ji * (2 * j - 3)).toDouble())
            }
        }
    }
    ...
    public override fun heightOffset(longitude: Double, latitude: Double, height: Double): Double {
        ...
        for (i in nmax downTo 0) {
            ...
            previousSht = sht
            sht = (-as1[i] * y * f1 * sht) + (s11[i] * cr[i]) + (s12[i] * sr[i])
        }
